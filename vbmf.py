# from https://github.com/CasvandenBogaard/VBMF
...
def rank_selection(tensor: np.array):
    if tl.backend.BackendManager.get_backend() != 'numpy':
        tl.backend.BackendManager.set_backend('numpy')

    unfold1 = unfold(tensor, mode=0)
    _, diag1, _, _ = EVBMF(unfold1)
    rank1 = diag1.shape[0]

    unfold2 = unfold(tensor, mode=1)
    _, diag2, _, _ = EVBMF(unfold2)
    rank2 = diag2.shape[0]
